...
When you open a folder/workspace that contains multiple repositories, the Source Control Graph view title shows a repository picker. By default, the Source Control Graph view shows the active repository, matching the information in the Status Bar. You can use the repository picker to _lock_ the Source Control Graph view to a particular repository.
...
- Use _Account1_ for Settings Sync and _Account2_ for the GitHub Pull Request extension
- Use _Account1_ for the GitHub extension (to push) and _Account2_ for GitHub Copilot
...
1. The ability for extensions to register a _tool_. A tool is a piece of functionality that is meant to be used by language models. For example, reading the Git history of a file.
