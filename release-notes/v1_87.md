...
Users can customize the window title by using the <code codesetting="window.title">window.title</code> setting. This iteration, we added two new variables that can be used with this setting: `$\{activeRepositoryName\}
...
+   "testConfiguration": "$\{workspaceFolder\}
/.vscode-test.js",
...
We have a new API that enables extensions to contribute _visualizers_ for a variable. These visualizers might be either actions that run a command, such as opening a new editor, or trees embedded inline into the Debug views and that take the place of the data's default representation.
