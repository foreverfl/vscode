...
Nesting under `index.ts` when a file matches the directory's name (`"index.ts": "$\{dirname\}
.ts"`):

![in directory named SomeFolder, SomeFolder.ts is nested under index.ts](images/1_67/nest-dirname.png)

Nesting files that have the same name as a different file but with an added segment (`"*": "$\{basename\}
.*.$\{extname\}
"`):
