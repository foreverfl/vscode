...
variable for task and debug configurations

A new variable `$\{relativeFileDirname\}
` resolves to the workspace folder relative path of the current open file's directory. The variable can be used in task and debug configurations (`tasks.json` and `launch.json`).
...
<body>
    <img src="${panel.webview.toWebviewResource(
      imagePath
    )}/cat.gif" width="300" />
</body>
</html>`;
