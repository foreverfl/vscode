...
With the introduction of a new variable `$\{defaultBuildTask\}
`, it is now possible to have a generic `preLaunchTask` that always refers to the default build task:
...
`$\{defaultBuildTask\}
` resolves to the name of the task that is run when the command **Tasks: Run Build Task** is executed. Similar to using the **Run Build Task** command, if there are multiple default build tasks, or no default build task, then a Quick Pick is shown to select the build task.
...
`)
      ...
      vscode.window.showInformationMessage(`Resolved to: $\{resolved\}
 on client and copied to clipboard`);
