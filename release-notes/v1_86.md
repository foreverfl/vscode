...
The new `$\{/\}
` variable can be used as a shorthand for the existing `$\{pathSeparator\}
` variable.
...
- `$\{src\}
` — The resolved path of the audio/video file.
- `$\{title\}
` — The title used for the audio/video. A snippet placeholder will automatically be created for this variable.
