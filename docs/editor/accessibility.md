...
By default, pressing `Tab` within a source code file inserts the Tab character (or spaces depending on your indentation setting) and does not leave the open file. You can toggle `Tab` trapping with `kb(editor.action.toggleTabFocusMode)`, and subsequent `Tab` keys will move focus out of the file. When default `Tab` trapping is off, you will see a **Tab moves focus** indicator in the Status Bar.
...
Read-only files never trap the `Tab` key. The **Integrated Terminal** panel respects the `Tab` trapping mode and can be toggled with `kb(editor.action.toggleTabFocusMode)`.
...
There is an Accessible Diff Viewer in the Diff editor that presents changes in a unified patch format. You can navigate between changes with **Go to Next Difference** (`F7`) and **Go to Previous Difference** (`kb(editor.action.accessibleDiffViewer.prev)`). Navigate lines with the arrow keys and press `Enter` to jump back in the Diff editor and the selected line.
