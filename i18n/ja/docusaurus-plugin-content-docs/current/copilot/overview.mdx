...
- **터미널 명령 수정 제안**. 터미널에서 명령 실행이 실패할 때, Copilot은 거터에 발생한 일을 설명하는 빠른 수정을 제공하는 sparkle을 표시합니다.
...
- **AI 생성 커밋 메시지 및 PR 설명** 커밋의 코드 변경 사항이나 풀 리퀘스트의 변경 사항을 기반으로. 소스 제어 뷰나 GitHub PR 확장의 _sparkle_ 버튼을 사용하여 변경 사항을 요약하는 제목과 설명을 생성합니다.

  ![소스 제어 입력 상자 sparkle 버튼에 마우스를 올리면 커밋 메시지 생성이 표시됩니다](images/overview/generate-commit-message.png)
