...
Start by selecting **Remote-SSH: Add New SSH Host...** from the Command Palette (`kbstyle(F1)`, `kb(workbench.action.showCommands)`) or clicking on the **Close Remote Connection* icon in the SSH **Remote Explorer** in the Activity Bar.
