...
스키마가 있는 JSON 데이터와 없는 JSON 데이터 모두에 대해 속성과 값에 대한 제안을 입력하는 동안 IntelliSense를 통해 제공합니다. 또한 **제안 트리거** 명령(`kb(editor.action.triggerSuggest)`)을 사용하여 수동으로 제안을 볼 수 있습니다.
...
JSON 파일은 커질 수 있으며, **기호로 이동** 명령(`kb(workbench.action.gotoSymbol)`)을 사용하여 속성으로 빠르게 탐색할 수 있습니다.

![기호로 이동](images/json/gotosymbol.png)

## 호버 {#hovers}

스키마가 있는 JSON 데이터와 없는 JSON 데이터의 속성과 값을 호버할 때 추가적인 컨텍스트를 제공합니다.

![호버](images/json/hoverandtoggle.png)

## 포맷팅 {#formatting}

`kb(editor.action.formatDocument)` 또는 컨텍스트 메뉴에서 **문서 포맷**을 사용하여 JSON 문서를 포맷할 수 있습니다.
...
[JSON 사양](https://www.json.org/)을 따르는 기본 JSON 모드 외에도, VS Code에는 **주석이 있는 JSON**(jsonc) 모드가 있습니다. 이 모드는 `settings.json`, `tasks.json`, 또는 `launch.json`과 같은 VS Code 구성 파일에 사용됩니다. **주석이 있는 JSON** 모드에서는 JavaScript에서 사용되는 단일 행(`//`) 및 블록 주석(`/* */`)을 사용할 수 있습니다. 이 모드는 후행 쉼표도 허용하지만, 권장되지 않으며 편집기에서 경고를 표시합니다.
...
JSON 파일을 스키마에 연결하는 것은 JSON 파일 자체에서 `$schema` 속성을 사용하거나 사용자 또는 작업 공간 [설정](/docs/editor/settings.md) (**파일** > **환경 설정** > **설정**)에서 `setting(json.schemas)` 속성 아래에서 수행할 수 있습니다.
...
>**팁:** `.babelrc`에 대한 스키마를 정의하는 것 외에도, `.babelrc`가 JSON 언어 모드에 연결되어 있는지 확인하세요. 이는 `files.association` 배열 설정을 사용하여 설정에서 수행됩니다.
...
```json
{
    ...
    "items": {
        ...
        "defaultSnippets": [
            {
                "label": "새 키보드 단축키",
                "description": "주어진 상태에 대한 명령에 키를 바인딩합니다.",
